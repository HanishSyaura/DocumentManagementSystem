...
export default function NewDocumentRequest() {
  ...
  const [loadingTemplateRequests, setLoadingTemplateRequests] = useState(true)
  ...
  const [canManageTemplateRequests, setCanManageTemplateRequests] = useState(false)
  ...
  useEffect(() => {
    try {
      ...
      const roleNames = roles.map((r) => r?.role?.name || r?.name).filter(Boolean)
      setCanManageTemplateRequests(roleNames.includes('admin') || roleNames.includes('document_controller'))
    } catch {
      setCanManageTemplateRequests(false)
    }
  }, [])
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="card p-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <h2 className="text-lg font-semibold text-gray-900">Template Requests</h2>
            <p className="text-sm text-gray-600 mt-1">
              Request a new template or ask to update an existing template. Admin / Document Controller can resolve requests.
            </p>
          </div>
          <button
            type="button"
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {loadingTemplateRequests ? (
                <tr>
                  <td colSpan={canManageTemplateRequests ? 7 : 5} className="text-center py-8 text-gray-500">
                    Loading...
                  </td>
                </tr>
