            ...
            <div className={`relative ${landingContent?.workflowImage && landingContent?.workflowImagePosition === 'left' ? 'order-1 md:order-2' : ''}`}>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl mx-auto">
                {workflowSteps.map((stepObj, idx) => {
                  ...
                  const colSpan = idx === workflowSteps.length - 1 && workflowSteps.length % 3 === 1 ? 'md:col-span-3' : ''

                  return (
                    <div key={idx} className={`${colSpan} relative group`}>
                      <div className={`absolute inset-0 bg-gradient-to-br ${style.from} ${style.to} rounded-xl blur-sm opacity-75 group-hover:opacity-100 transition`}></div>
                      <div className={`relative bg-white rounded-xl p-4 shadow-lg border ${style.border} hover:scale-105 transition-all`}>
                        <div className="flex items-center gap-3">
                          <div className={`w-9 h-9 bg-gradient-to-br ${style.badgeFrom} ${style.badgeTo} rounded-full flex items-center justify-center text-white font-bold shadow-md text-sm`}>
                            {idx + 1}
                          </div>
